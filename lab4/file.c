...
static ssize_t osfs_write(struct file *filp, const char __user *buf, size_t len, loff_t *ppos) {
    ...
    if (osfs_inode->i_blocks == 0) {
        ret = osfs_allocate_block(sb_info, osfs_inode);
        if (ret < 0)
            return ret;
    }
    ...
    osfs_inode->i_size = max(osfs_inode->i_size, *ppos + len);
    osfs_inode->i_ctime = get_seconds();
    osfs_inode->i_mtime = osfs_inode->i_ctime;
    osfs_inode->i_blocks = (osfs_inode->i_size + sb_info->block_size - 1) / sb_info->block_size;
